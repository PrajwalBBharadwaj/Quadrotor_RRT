...
class RRT():
    def __init__(self,obstacles_list):
        ...
        '''
        self.nodes = -1*np.ones((8001,6)) # x, y, z, cost of segment b/w node and parent, parent, in_tree(1/0), id_number
    ...
    def sample(self):
        ...
        '''
        while True:
            x = 10*np.random.uniform(-1,1,size=(3))
            x_point = Point(x)
    ...
    def get_cost(self,index):
        ...
        if index is not None:
            while True:
                cost = cost + self.nodes[index][3]
                index = int(self.nodes[index][4])
                if(self.nodes[index][4]==self.p_current):
                    return cost
            return None
        else:
            return None
    ...
    def steer(self, x1, nearest):
        ...
        if distance>1:
            ...
            x_new[2] = nearest[2] + (x1[2]-nearest[2])/distance
            return x_new,1
        else:
            return x1,distance
    ...
    def nearest(self, x_rand):
        ...
        '''
        x_nearest_dist = np.linalg.norm(self.nodes[0:self.nodes_num,0:3]-x_rand,axis=1) #NOTE: I haven't added the memeber to the tree yet
    ...
    def get_near_list(self, x_rand):
        ...
        '''
        x_nearest_dist = np.linalg.norm(self.nodes[0:self.nodes_num,0:3]-x_rand,axis=1)
        x_nearest_ids = np.argsort(x_nearest_dist)
    ...
    def nearest_from_list(self, x_rand, near_indices, reconnect):
        ...
        for k,index in enumerate(near_indices):
            condition = self.is_in_collision(x_rand,self.nodes[index][0:3])
            
            if condition is False:
                cost[k] = self.get_cost(index)
                cost[k] = cost[k] + np.linalg.norm(x_rand-self.nodes[index][0:3])
            else:
                cost[k] = 1000000
    ...
    def connect(self, x, dist_index, cost_steer, new = False):
        ...
        '''
        if new is True:
            self.nodes[self.nodes_num][0:3] = x
            self.nodes[self.nodes_num][3] = cost_steer # cost of that node
            self.nodes[self.nodes_num][4] = dist_index  # parent index
            self.nodes[self.nodes_num][5] = 1 # in tree condition
            
            self.nodes_num +=1 # adding up the total number of nodes
        else:
            self.nodes[self.nodes_num-1][0:3] = x
            self.nodes[self.nodes_num-1][3] = cost_steer # cost of that node
            self.nodes[self.nodes_num-1][4] = dist_index  # parent index
    ...
    def rewire(self, x_rand, near_indices):
        ...
        for i in near_indices:
            if self.nodes[i,4] == self.p_current:
                continue
            condition = self.is_in_collision(x_rand,self.nodes[i][0:3])
            if condition is False:
                dist = np.linalg.norm(x_rand-self.nodes[i][0:3])
                cost_near = cost_new + dist
                if cost_near<self.get_cost(i):
                    self.nodes[i][4] = self.nodes_num-1
                    self.nodes[i][3] = dist
    ...
    def check_goal(self):
        ...
        dist_from_goal = np.linalg.norm(self.nodes[0:self.nodes_num,0:3]-self.goal,axis=1)
    ...
    def find_path(self, min_cost_idx):
        ...
        while min_cost_idx!=self.p_current:
            p = int(self.nodes[min_cost_idx][4])
            path.append(p)
            min_cost_idx = p
        return path
    ...
    def plot(self, path=0, path_present=False):
        ...
        for i in range(self.nodes_num):
            if self.nodes[i][5]==1:
                parent = int(self.nodes[i][4])
                x = [self.nodes[i][0],self.nodes[parent][0]]
                y = [self.nodes[i][1],self.nodes[parent][1]]
                z = [self.nodes[i][2],self.nodes[parent][2]]
                ax.plot(x,y,z,color='cyan',linewidth=1,zorder=-1)
        if path_present is True:
            ...
            for i in path:
                ...
                points_z.append(self.nodes[i][2])
        ...
        ax.scatter(self.start[0],self.start[1],self.start[2],s=10**2,color='blue',zorder=1)
        ax.scatter(8,8,8,s=10**2,color='green',zorder=1)
        plt.show()